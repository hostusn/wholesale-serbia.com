...
<div class="tribe-venue-widget-wrapper">
	<div class="tribe-venue-widget-venue">
		<?php if (has_post_thumbnail($venue_ID)) { ?>
			<div class="tribe-venue-widget-thumbnail">
				<?php echo get_the_post_thumbnail($venue_ID, 'event-thumb' ); ?>
			</div>
		<?php } ?>
	...
	<?php if ( 0 === $events->post_count ): ?>
		<?php printf( esc_html__( 'No upcoming %s.', 'mall' ),  strtolower( $events_label_plural ) ); ?>
	<?php else: ?>
	...
	<p class="tribe-events-widget-link">
		<a href="<?php echo esc_url( tribe_get_venue_link( $venue_ID, false ) ); ?>"><?php printf( esc_html__( 'View all %s at this %s', 'mall' ), $events_label_plural, tribe_get_venue_label_singular() ); ?></a>
	</p>
</div>
