...
<div class="tribe-mini-calendar-event event-<?php echo esc_attr( $mini_cal_event_atts['current_post'] ); ?> <?php echo esc_attr( $mini_cal_event_atts['class'] ); ?>">
	...
	<div class="tribe-events-list-widget-content-wrap">
		...
		<div class="cmsmasters_widget_event_info">
			...
			?>
				<div class="cmsmasters_widget_event_venue_loc <?php if ( 
					( isset( $organizer ) && $organizer && empty( $organizer_ids ) )
					) { 
						echo 'cmsmasters_theme_icon_person">';
					} else if ( isset( $phone ) && $phone && tribe_get_phone() != '' ) {
						echo 'cmsmasters_theme_icon_user_phone">';
					} else {
						echo '">';
					}
					?>
					<?php if ( isset( $organizer ) && $organizer && empty( $organizer_ids ) ): ?>
						<span class="tribe-events-organizer">
							...
							$and = _x( 'and', 'list separator for final two elements', 'mall' );
							if ( 1 == count( $organizer_links ) ) {
								echo cmsmasters_return_content($organizer_links[0]);
							}// end if
							elseif ( 2 == count( $organizer_links ) ) {
								echo cmsmasters_return_content($organizer_links[0] . ' ' . esc_html( $and ) . ' ' . $organizer_links[1]);
							}// end elseif
							else {
								...
								echo esc_html( ', ' . $and . ' ' );
								echo cmsmasters_return_content($last_organizer);
							}// end else
							?>
						</span>
					<?php endif ?>
